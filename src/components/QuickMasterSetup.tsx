...
export default function QuickMasterSetup({
  ...
  return (
    <div id="quick-master-setup-root" className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      {/* ========================================================================= */}
      {/* QUICK COMPLIANCE POLICY SETUP MODULE */}
      {/* ========================================================================= */}
      <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-sky-950 p-6 rounded-3xl border border-indigo-500/30 shadow-xl text-white space-y-6 relative overflow-hidden">
        <div className="absolute -right-10 -bottom-10 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-indigo-500/20 pb-4 relative z-10">
          <div className="space-y-1">
            <span className="px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-400/30 text-[10px] font-black uppercase tracking-wider font-mono inline-flex items-center gap-1.5">
              <Wrench className="w-3.5 h-3.5 text-indigo-400" /> Compliance Governance Framework
            </span>
            <h2 className="text-xl font-black text-white mt-2 flex items-center gap-2">
              <Shield className="w-6 h-6 text-indigo-400" />
              Quick Compliance Policy Setup
            </h2>
            <p className="text-xs text-slate-300 max-w-3xl leading-relaxed">
              Configure and publish 7 critical compliance policies instantly tailored for your facility setup. Select team sign-off roles, customize antivirus software and access review schedules, then generate customized policy documents.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
            <button
              ...
              className="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/50 cursor-pointer hover:scale-[1.02]"
            >
            ...
            {onNavigateTab && (
              <button
                ...
                className="px-4 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-extrabold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
                View Frameworks <ChevronRight className="w-4 h-4 text-indigo-400" />
              </button>
            )}
          </div>
        </div>

        {/* 7 Critical Policies Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 relative z-10">
          {[
            { code: 'POL-SEC-009', name: 'Antivirus & Endpoint Security', desc: 'Endpoint protection, patch updates & Defender EDR', icon: <Shield className="w-4 h-4 text-emerald-400" /> },
            { code: 'POL-SEC-006', name: 'Access Control & Privileges', desc: 'Role permissions, MFA & periodic clinical access audits', icon: <Lock className="w-4 h-4 text-indigo-400" /> },
            { code: 'POL-SEC-013', name: 'Physical & Environmental Security', desc: 'Secure zones, server room vaults & visitor logs', icon: <Server className="w-4 h-4 text-cyan-400" /> },
            { code: 'POL-SEC-021', name: 'Backup & Data Restoration SOP', desc: 'Disaster recovery, air-gapped backups & retention', icon: <HardDrive className="w-4 h-4 text-amber-400" /> },
            { code: 'POL-SEC-025', name: 'Incident Management Policy', desc: 'Breach response, DOH notification & root cause analysis', icon: <AlertTriangle className="w-4 h-4 text-rose-400" /> },
            { code: 'POL-SEC-031', name: 'Document Control & Governance', desc: 'Version tracking, review cycles & master approval', icon: <FileText className="w-4 h-4 text-blue-400" /> },
            { code: 'M-Policy-002', name: 'Statement of Applicability (SoA)', desc: 'ADHICS & ISO 27001 control applicability matrix', icon: <Table className="w-4 h-4 text-purple-400" /> },
            { code: 'AUTHOR/CEO', name: 'Document Sign-off Team', desc: 'Author, Verifier & CEO electronic signature workflows', icon: <PenTool className="w-4 h-4 text-sky-400" /> },
          ].map((item) => (
            <div key={item.code} className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-700/60 hover:border-indigo-400/50 transition-all space-y-2">
              <div className="flex items-center justify-between">
                <div className="p-1.5 rounded-lg bg-slate-800 border border-slate-700">
                  {item.icon}
                </div>
                <span className="text-[9px] font-mono font-bold text-indigo-300 bg-indigo-500/20 px-2 py-0.5 rounded-full border border-indigo-400/30">
                  {item.code}
                </span>
              </div>
              <div>
